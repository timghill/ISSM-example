...
if alpha_s<3 && omega==0
    ...
    p_w_min = min(md.geometry.thickness)*md.materials.rho_ice*md.constants.g;
    p_w_max = max(md.geometry.thickness)*md.materials.rho_ice*md.constants.g;
    gradphi = (p_w_max - p_w_min)/100e3;
    omega_ref = 1/2000;
    nu = md.materials.mu_water/md.materials.rho_freshwater;
    h3 = nu/(omega_ref)/k_s/gradphi;
...
% Let this code work for old and new ISSM SVN revisions
try
    md.hydrology.rheology_B_base = md.materials.rheology_B;
catch
    fprintf('No attribute md.hydrology.rheology_B_base, continuing\n')
end
...
moulin_input = 10*max(0, -cos(2*pi*tt_melt));
